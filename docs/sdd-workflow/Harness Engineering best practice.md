...
- [Claude Code / Codex ユーザーのための誰でもわかるHarness Engineeringベストプラクティス](#claude-code--codex-ユーザーのための誰でもわかるharness-engineeringベストプラクティス)
  - [Harness Engineeringとは何か](#harness-engineeringとは何か)
    - [定義を辿る](#定義を辿る)
    - [この分野の寿命について](#この分野の寿命について)
    - [ハーネスへの投資は複利で効く](#ハーネスへの投資は複利で効く)
  - [1: リポジトリ衛生: 腐敗を前提に設計する](#1-リポジトリ衛生-腐敗を前提に設計する)
    - [置くべきもの](#置くべきもの)
    - [置くべきでないもの](#置くべきでないもの)
    - [リポジトリの衛生管理](#リポジトリの衛生管理)
    - [テストはドキュメントより腐敗に強い](#テストはドキュメントより腐敗に強い)
    - [ADRで決定の履歴を保全する](#adrで決定の履歴を保全する)
    - [現実的な解を追い求める](#現実的な解を追い求める)
  - [2: 決定論的ツールとアーキテクチャガードレールで品質を強制する](#2-決定論的ツールとアーキテクチャガードレールで品質を強制する)
    - [リンターの仕事をLLMにさせない](#リンターの仕事をllmにさせない)
    - [フィードバックループの設計: Hooksの活用](#フィードバックループの設計-hooksの活用)
    - [4つのHookパターン](#4つのhookパターン)
    - [実装例: PostToolUseによる自動リント(TypeScript/JavaScript)](#実装例-posttooluseによる自動リントtypescriptjavascript)
    - [Planktonパターン(高度)](#planktonパターン高度)
    - [言語別リンター選定ガイド(2026年3月)](#言語別リンター選定ガイド2026年3月)
      - [TypeScript/JavaScript: Oxlint(リント)+ Biome(フォーマット)](#typescriptjavascript-oxlintリント-biomeフォーマット)
      - [Python: Ruff(一択)](#python-ruff一択)
      - [Go: golangci-lint](#go-golangci-lint)
      - [Rust: Clippy(pedantic + allow\_attributes禁止)](#rust-clippypedantic--allow_attributes禁止)
      - [Swift / Kotlin](#swift--kotlin)
      - [リンター比較テーブル](#リンター比較テーブル)
    - [カスタムリンター戦略: エージェント向けルールの設計](#カスタムリンター戦略-エージェント向けルールの設計)
      - [Factory.aiの4カテゴリ](#factoryaiの4カテゴリ)
      - [カスタムルールの実装ツール選定](#カスタムルールの実装ツール選定)
    - [エラーメッセージを修正指示にする](#エラーメッセージを修正指示にする)
    - [プリコミットフックで即時フィードバックを提供する](#プリコミットフックで即時フィードバックを提供する)
    - [ADRと実行可能ルールを結合する](#adrと実行可能ルールを結合する)
    - [リンター設定保護: エージェントの「ルール改竄」を防ぐ](#リンター設定保護-エージェントのルール改竄を防ぐ)
    - [AI生成コード固有のリントアンチパターン](#ai生成コード固有のリントアンチパターン)
    - [言語別推奨リンタースタック](#言語別推奨リンタースタック)
      - [TypeScript/Node.jsプロジェクト](#typescriptnodejsプロジェクト)
      - [Pythonプロジェクト](#pythonプロジェクト)
      - [Goプロジェクト](#goプロジェクト)
      - [Rustプロジェクト](#rustプロジェクト)
    - [フィードバックの速度が品質を決定する](#フィードバックの速度が品質を決定する)
    - [アーキテクチャをガードレールにする](#アーキテクチャをガードレールにする)
  - [3: AGENTS.md / CLAUDE.mdをポインタとして設計する](#3-agentsmd--claudemdをポインタとして設計する)
    - [書くべきもの](#書くべきもの)
    - [書くべきでないもの](#書くべきでないもの)
    - [サイズの目安](#サイズの目安)
    - [ポインタが腐敗した場合](#ポインタが腐敗した場合)
  - [4: 計画と実行を分離する](#4-計画と実行を分離する)
    - [計画フェーズ](#計画フェーズ)
    - [タスクの粒度](#タスクの粒度)
    - [テストによる完了の検証](#テストによる完了の検証)
  - [5: E2Eテスト戦略: エージェントにあらゆるアプリの「目」を与える](#5-e2eテスト戦略-エージェントにあらゆるアプリの目を与える)
    - [Webアプリ: ツール比較(3つのアプローチ)](#webアプリ-ツール比較3つのアプローチ)
      - [1. Playwright MCP(Microsoft公式)](#1-playwright-mcpmicrosoft公式)
      - [2. Playwright CLI(`@playwright/cli`)](#2-playwright-cliplaywrightcli)
      - [3. agent-browser(Vercel Labs)](#3-agent-browservercel-labs)
      - [推奨: 用途別の選定](#推奨-用途別の選定)
    - [普遍的原則: アクセシビリティツリーはユニバーサルインターフェース](#普遍的原則-アクセシビリティツリーはユニバーサルインターフェース)
    - [アクセシビリティツリー vs スクリーンショット: 使い分け](#アクセシビリティツリー-vs-スクリーンショット-使い分け)
    - [モバイルアプリE2Eテスト](#モバイルアプリe2eテスト)
      - [状況(2026年3月)](#状況2026年3月)
      - [推奨ツールスタック](#推奨ツールスタック)
      - [モバイルE2Eの設計判断](#モバイルe2eの設計判断)
    - [CLI/TUIアプリケーションE2Eテスト](#clituiアプリケーションe2eテスト)
    - [API/バックエンドE2Eテスト](#apiバックエンドe2eテスト)
    - [デスクトップアプリE2Eテスト](#デスクトップアプリe2eテスト)
      - [Electronアプリ](#electronアプリ)
      - [Tauriアプリ](#tauriアプリ)
      - [ネイティブデスクトップアプリ](#ネイティブデスクトップアプリ)
      - [プラットフォーム別アクセシビリティAPI](#プラットフォーム別アクセシビリティapi)
    - [インフラストラクチャ/DevOps E2Eテスト](#インフラストラクチャdevops-e2eテスト)
      - [Terraform](#terraform)
      - [Docker](#docker)
      - [Kubernetes](#kubernetes)
      - [インフラE2Eの設計判断](#インフラe2eの設計判断)
    - [AI/MLパイプラインE2Eテスト](#aimlパイプラインe2eテスト)
      - [データパイプラインテスト](#データパイプラインテスト)
      - [モデル評価（ベンチマーク）](#モデル評価ベンチマーク)
      - [アプリケーション品質評価（LLM）](#アプリケーション品質評価llm)
      - [継続的評価（Continuous Evaluation）](#継続的評価continuous-evaluation)
      - [安全性テスト・ガードレール](#安全性テストガードレール)
      - [まとめ](#まとめ)
    - [ユニバーサルE2E原則のまとめ](#ユニバーサルe2e原則のまとめ)
    - [アニメーション・トランジションの検証戦略](#アニメーショントランジションの検証戦略)
      - [レイヤード検証戦略](#レイヤード検証戦略)
      - [Layer 1: getAnimations() APIによる決定論的検証](#layer-1-getanimations-apiによる決定論的検証)
      - [Layer 2: CLS(Cumulative Layout Shift)の計測](#layer-2-clscumulative-layout-shiftの計測)
      - [Layer 3: ビジュアルリグレッション(アニメーション凍結)](#layer-3-ビジュアルリグレッションアニメーション凍結)
      - [Layer 4: 低FPSフレームキャプチャによるエージェント視覚検証](#layer-4-低fpsフレームキャプチャによるエージェント視覚検証)
      - [フィードバックループへの統合](#フィードバックループへの統合)
  - [6: セッション間の状態管理を設計する](#6-セッション間の状態管理を設計する)
    - [問題の構造](#問題の構造)
    - [起動ルーチンを標準化する](#起動ルーチンを標準化する)
    - [Gitをセッション間のブリッジとして使う](#gitをセッション間のブリッジとして使う)
    - [進捗記録にJSONを使う](#進捗記録にjsonを使う)
  - [7: プラットフォーム固有のハーネス戦略を理解する: Codex vs Claude Code](#7-プラットフォーム固有のハーネス戦略を理解する-codex-vs-claude-code)
    - [ハーネスがモデルより重要](#ハーネスがモデルより重要)
    - [アーキテクチャの根本的違い](#アーキテクチャの根本的違い)
    - [Codex固有のハーネス機能](#codex固有のハーネス機能)
    - [Claude Code固有のハーネス機能](#claude-code固有のハーネス機能)
    - [両方で可能だがアプローチが異なるもの](#両方で可能だがアプローチが異なるもの)
    - [ハイブリッド戦略: 計画はClaude Code、実行はCodex](#ハイブリッド戦略-計画はclaude-code実行はcodex)
    - [意思決定フレームワーク](#意思決定フレームワーク)
  - [アンチパターン](#アンチパターン)
  - [最小実行可能ハーネス(MVH: Minimum Viable Harness)](#最小実行可能ハーネスmvh-minimum-viable-harness)
    - [Week 1](#week-1)
    - [Week 2-4](#week-2-4)
    - [Month 2-3](#month-2-3)
    - [Month 3+](#month-3)
  - [まとめ](#まとめ-1)
  - [2026-03-11 修正](#2026-03-11-修正)
  - [References](#references)
    - [一次ソース](#一次ソース)
    - [Claude Code / Codex](#claude-code--codex)
    - [リンターツール](#リンターツール)
    - [E2Eテスト](#e2eテスト)
    - [インフラ / DevOps](#インフラ--devops)
    - [AI/ML](#aiml)
      - [データ品質](#データ品質)
      - [モデル評価・ベンチマーク](#モデル評価ベンチマーク-1)
      - [LLM評価・CI/CD統合](#llm評価cicd統合)
      - [安全性・ガードレール](#安全性ガードレール)
      - [可観測性・ドリフト検出](#可観測性ドリフト検出)
    - [その他](#その他)
...
### その他
  ...
  * [ ← 前の記事 **Coding Agent時代に、開発速度を遅くする** ](/posts/not-building-in-agent-era/)
  * [ 次の記事 → **Coding Agent時代の開発ワークフローについてのまとめ** ](/posts/coding-agent-workflow-2026/)
