...
`tasklist.md`の内容に基づいてTodoWriteツールでタスクリストを作成:
...
4. **コミットを推奨（セッション間の継続性確保）**

   フェーズ完了時はコミットを提案する:
   ```
   「フェーズ1の実装が完了しました。この時点でコミットしておくと、
   セッションが中断した場合でも /resume-work で正確に再開できます。
   コミットしますか？」
   ```
   ユーザーが承認した場合のみコミットを実行する。
...
### 目的
実装完了後、tasklist.mdに振り返りを記録します。作業を中断する場合は Handoff セクションも記録します。
...
2. **完了/中断を判定**
   - **全タスク `[x]`（完了）**: ステップ3（振り返り）へ進む
   - **未完了タスクあり（中断）**: ステップ2.5（Handoff 記録）を先に行う

2.5. **【中断時のみ】Handoff セクションを記録**

   tasklist.md の「Handoff（作業中断・引き継ぎ記録）」セクションを Editツールで更新:
   - 中断日時
   - 完了済みフェーズ・タスク（`[x]` 済みのまとめ）
   - **次に着手すべきタスク**（最重要: 再開時の入口）
   - 中断時の状態（未確定事項・ブロッカー）
   - 注意事項・リスク

   このセクションが `/resume-work` の Step 3 で優先読み込みされる。
