...
大きな変更（ファイル追加・ディレクトリ追加・リファクタリング等）の後に確認するセクション。小規模変更の場合はスキップ可。

**チェック項目**:
- [ ] ゴーストファイルがないか（`_old`, `_v2`, `_backup` サフィックス、未使用の類似名ファイル）
- [ ] 命名が `docs/glossary.md` の定義と整合しているか（存在する場合）
- [ ] 循環依存がないか（モジュール A が B をインポートし、B が A をインポートするなど）
- [ ] 一時コード・デバッグログが残っていないか（`console.log`, `print`, `[DEBUG-*]` タグ等）
- [ ] ADR 化すべきアーキテクチャ意思決定が暗黙的に行われていないか

**評価基準**:
- ✅ 整理済み: ゴーストファイル・一時コードなし、命名整合
- ⚠️ 要整理: 軽微な残骸あり
- ❌ 要対応: ゴーストファイル多数・循環依存・デバッグコード混入
...
上記6つの観点（スペック準拠、コード品質、テストカバレッジ、セキュリティ、パフォーマンス、構造レビュー）から検証します。ただし構造レビューは小規模変更の場合スキップ可。
